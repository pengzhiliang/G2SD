...
def get_num_layer_for_vit_distill(var_name, num_max_layer):
    if var_name in ("0.backbone.cls_token", "0.backbone.mask_token", "0.backbone.pos_embed"):
        return 0
    elif var_name.startswith("0.backbone.patch_embed"):
        return 0
    elif var_name.startswith("0.backbone.blocks"):
...
@OPTIMIZER_BUILDERS.register_module()
class LayerDecayOptimizerConstructor_distill(DefaultOptimizerConstructor):
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        """Add all parameters of module to the params list.
        The parameters of the given module will be added to the list of param
        groups, with specific rules defined by paramwise_cfg.
        ...
        for name, param in module.named_parameters():
            ...
            layer_id = get_num_layer_for_vit_distill(name, num_layers)
            group_name = "layer_%d_%s" % (layer_id, group_name)
