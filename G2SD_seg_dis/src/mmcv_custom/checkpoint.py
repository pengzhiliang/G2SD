...
def load_checkpoint(model,
    ...
    # interplote patch embedding weight; patch size 14 -> patch size 16
    if state_dict['patch_embed.proj.weight'].shape[-1] == 14:
        if model.state_dict()['patch_embed.proj.weight'].shape[-1] == 16:
            print(f'Expand the patch embedding weight from p14 to p16')
            new_conv = F.interpolate(state_dict['patch_embed.proj.weight'], size=(16,16), mode='bicubic', align_corners=False)
            state_dict['patch_embed.proj.weight'] = new_conv
