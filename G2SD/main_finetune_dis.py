...
from engine_finetune_dis import train_one_epoch, evaluate
import utils
def bool_flag(s):
    """
...
def get_args_parser():
    ...
    parser.add_argument('--teacher_path', default='',
                        help='finetune from checkpoint')
    parser.add_argument('--teacher_model',default='vit_large_patch16', type=str, metavar='MODEL',
                        help='Name of model to teacher')
    parser.add_argument('--enable_deepspeed', action='store_true', default=False)
    parser.add_argument('--enable_nkd',  default=False,type=bool_flag)
...
def main(args, ds_init):
    ...
    model = models_vit.__dict__[args.model](
        ...
        global_pool=args.global_pool,distillation=True,use_distoken= not args.enable_nkd
    )
   
    model_teacher = models_vit.__dict__[args.teacher_model](
        num_classes=args.nb_classes,
        drop_path_rate=args.drop_path,
        global_pool=True,
    )
    ...
    if args.teacher_path:
        ...
        print("Load pre-trained teacher checkpoint from: %s" % args.teacher_path)
        checkpoint_model = checkpoint['model']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir or epoch%40==0 or epoch==args.epochs:
            misc.save_model(
