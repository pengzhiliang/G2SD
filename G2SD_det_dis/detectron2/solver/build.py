...
def build_lr_scheduler_distill(
    cfg: CfgNode, optimizer: torch.optim.Optimizer
) -> torch.optim.lr_scheduler._LRScheduler:
    """
    Build a LR scheduler from config.
    """
    # name = cfg.lr_multiplier
    ...
    sched = MultiStepParamScheduler(
            values=cfg.lr_multiplier.scheduler['values'],
            milestones=cfg.lr_multiplier.scheduler['milestones'],
            num_updates=cfg.train.max_iter,
        )
    ...
    sched = WarmupParamScheduler(
        sched,
        0.001,
        250 / cfg.train.max_iter,
    )
    return LRMultiplier(optimizer, multiplier=sched, max_iter=cfg.train.max_iter)
